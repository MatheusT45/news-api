...
use JWTAuth;
use DB;
@define('EMAILUNIT',rand(10,10000).'@news.com');
class AdminTest extends TestCase
{
    public function test_admin_add()
     {
         $response1 = $this->postJson(action([AuthController::class, 'adminAdd']), [
             'name'     => 'Admin1',
             'email'    => EMAILUNIT,
             'password' => 123456,
         ])->assertStatus(201)->getOriginalContent();

         $response = $this->postJson(action([AuthController::class, 'adminAdd']), [
             'name'     => 'Admin1',
             'password' => 123456,
         ])->assertStatus(400);
	...
	public function test_admin_add_invalid_error()
    {
        $response = $this->postJson(action([AuthController::class, 'adminAdd']), [
            'name'     => 'Admin1',
            'password' => 123456,
     ...
     {
        $response1 = $this->postJson(action([AuthController::class, 'login']), [
         	'email'	   => EMAILUNIT,
         	'password' => '123456',
        ]);

        $token = auth()->attempt(['email' => EMAILUNIT, 'password' => '123456']);
        if(!$token)
		{
          	$this->assertEquals($response1['errMsg'], 'incorrect email or password');
        }
        @define('TOKEN', $token);
	...
	public function test_admin_login()
    {
		...
		$response1 = $this->postJson(action([AuthController::class, 'newsAdd']), [
    ...
    public function test_add_news()
    {
        $headers = [
         ...
         $response2 = $this->postJson(action([AuthController::class, 'newsAdd']), [
